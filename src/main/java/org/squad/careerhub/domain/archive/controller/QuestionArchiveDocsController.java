...
public abstract class QuestionArchiveDocsController {
    ...
    @Operation(
            summary = "지원서 내 아카이브된 질문 목록 조회 - [JWT O]",
            description = """
                    ### 특정 지원서에 아카이브된 질문 목록을 조회합니다.
                    - 대상: 특정 지원서(applicationId)에 연결된 아카이브 질문들
                    - 지원서 상세 페이지에서 '면접 질문 보기' 기능에 사용됩니다.
                    
                    - 요청 파라미터:
                      - lastCursorId: 마지막으로 조회한 질문 ID (optional) null일 경우 첫 페이지
                      - size: 페이지 크기 (optional, default = 10)
                    - 응답:
                        - contents: 아카이브된 질문 목록
    ...
    public abstract ResponseEntity<PageResponse<ApplicationQuestionArchiveResponse>> getArchivedQuestionsFromApplication(
            Long applicationId,
            Long lastCursorId,
            int size,
            Long memberId
